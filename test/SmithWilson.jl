@testset "SmithWilson" begin
    ...
    cfs = [1 0 0
        0 1 0
        0 0 1]
    obs = ZCBPrice.(prices, times)

    curve_three = curve(w,obs)
    ...
    cfs = [0.1 0.1
        ...
        0.0 1.0]
    obs = [
        Quote(1.0,Cashflow(0.1,times[1]) + Cashflow(1.0,times[2])),
        Quote(0.9,Cashflow(0.1,times[1]) + Cashflow(0.1,times[2]) + Cashflow(1.0,times[3]))
    ]
    curve_nondiag = curve(w, obs)
