@testset "CairnsPritchard" begin
    ...
    @testset "Fit to Cairns (1998) Figure 1 data" begin
        ...
        mats = [
            0.4511004586929275, 0.850176783219295, 1.2049112939094009, 1.6052006163826604,
    ...
    @testset "Fit to Bank of England gilt data (2 components)" begin
        ...
        gilt = [
            4.38, 4.83, 5.09, 5.21, 5.26, 5.26, 5.24, 5.2, 5.16, 5.11,
            5.07, 5.03, 4.98, 4.95, 4.91, 4.88, 4.86, 4.83, 4.82, 4.8,
            4.79, 4.78, 4.77, 4.75, 4.74, 4.73, 4.72, 4.7, 4.69, 4.67,
            4.66, 4.64, 4.62, 4.61, 4.59, 4.56, 4.54, 4.52, 4.5, 4.47,
            4.45, 4.42, 4.4, 4.37, 4.34, 4.31, 4.28, 4.26, 4.23, 4.2,
        ] ./ 100
    ...
    @testset "zero(model, 0) analytical limits" begin
        ...
        @test FinanceCore.rate(FinanceModels.zero(cp, 1.0e-10)) ≈ FinanceCore.rate(FinanceModels.zero(cp, 0.0)) atol = 1.0e-6
        @test FinanceCore.rate(FinanceModels.zero(cpe, 1.0e-10)) ≈ FinanceCore.rate(FinanceModels.zero(cpe, 0.0)) atol = 1.0e-6
    end
