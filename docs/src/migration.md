...
- **`Yield.FlatForwardAt(f)` supplies an independent terminal forward.** The continuously-compounded rate `f` stays fixed through fitting and Accessors updates. It preserves the final-knot discount factor and usually introduces a forward jump. The default `:flat_forward` instead recomputes its anchor from the chosen interpolant's endpoint derivative; changing interpolants can materially change tail levels and sensitivities (see [Interpolation Methods](interpolation.md)).
- **MonotoneConvex supports tail policies natively.** Direct construction and `fit(Spline.MonotoneConvex(), …)` return a native `Yield.MonotoneConvex` for `:flat_forward`, `:flat_zero`, `:linear`, and `Yield.FlatForwardAt(f)`, including the `Fit.Bootstrap()` descriptor path. `.rates`, `.times`, and `Yield.instantaneous_forward` remain available. Its concrete type gains parameters for the policy and derived tail; dispatch on the curve family rather than assuming its complete parameter list. At the final knot, `instantaneous_forward` reports the interior (left) value.
