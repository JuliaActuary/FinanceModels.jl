...
- **The interpolation cache is internal**: `propertynames(zrc)` is `(:rates, :tenors, :spline, :extrapolation)`, `zrc._model` throws, and `@set zrc._model = …` throws. The undocumented 4-argument `ZeroRateCurve(rates, tenors, spline, model)` constructor is removed; `ConstructionBase.setproperties`/`constructorof` reconstruct through the public constructor. This is Julia's conventional privacy: raw `getfield` access still reaches the fields and is unsupported.
...
- **`fit` validates the knot grid before optimising**: `fit(::Spline.SplineCurve, quotes, Fit.Loss(…))`, `fit(Yield.MonotoneConvex(), quotes)` and `Fit.Bootstrap()` throw the construction `ArgumentError` for duplicate/non-positive maturities or too few quotes for the interpolant (e.g. a loss fit of two quotes with `Spline.PCHIP()`) up front, rather than failing inside the solver. Optimizer trial curves are built without validation (a non-finite candidate is a bad loss, not an exception); the returned curve is validated, so a fit that diverged to non-finite rates now throws instead of returning a curve of `NaN`s.
...
!!! warning "Changed numbers: zero rates beyond the last knot"
    Every DataInterpolations-backed curve and fit now extrapolates with `:flat_forward` by default instead of continuing its final interpolation piece. This changes zero rates, discount factors and present values **beyond the last knot** for `Yield.Spline`, `Yield.build_model`, `ZeroRateCurve` and `fit` (loss fits and `Fit.Bootstrap()`) with `Spline.Linear()`, `Quadratic()`, `Cubic()`, `PCHIP()`, `Akima()` and `BSpline(d)`. Values at and before the last knot do not change. `Spline.MonotoneConvex()`, the `ZeroRateCurve` default, keeps its v6.1 tail and does not change.
    ...
    **To keep the previous values**, pass `extrapolation = :extension` wherever the curve is built: `Yield.Spline(spline, tenors, rates; extrapolation = :extension)`, `ZeroRateCurve(rates, tenors, spline; extrapolation = :extension)`, `Yield.build_model(spline, tenors, rates; extrapolation = :extension)`, or `fit(spline, quotes, method; extrapolation = :extension)`.

- **Knot curves take an `extrapolation` policy for the long end.** `Yield.Spline`, `ZeroRateCurve`, `Yield.MonotoneConvex`, `Yield.build_model` and the spline `fit` methods accept `extrapolation = :flat_forward` (default), `:flat_zero`, `:linear`, `:extension`, or `Yield.FlatForwardAt(rate)`. `:extension` restores the v6 continuation of the final DataInterpolations piece and is unavailable for `Spline.MonotoneConvex()`.
...
- **`zrc.extrapolation` and `c.extrapolation` are public properties** of `ZeroRateCurve` and `Yield.MonotoneConvex`, preserved by `fit` and `Accessors.@set`; `ConstructionBase.getproperties` includes them. `ZeroRateCurve` gains a type parameter for the policy, and `Yield.MonotoneConvex`'s type parameters are internal: dispatch on the type name and use its documented properties and methods.
...
- **`Yield.MonotoneConvex` equality is structural.** `==`, `isequal`, and `hash` use `(rates, times, extrapolation)`, excluding derived caches. Independently constructed curves with equal public inputs compare equal; `isequal` keeps the signed-zero distinction of the underlying rates. `ZeroRateCurve` equality also compares `extrapolation`.
