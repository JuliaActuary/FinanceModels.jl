...
- **The interpolation cache is internal**: `propertynames(zrc)` is `(:rates, :tenors, :spline, :extrapolation)`, `zrc._model` throws, and `@set zrc._model = …` throws. The undocumented positional cache constructor is removed; `ConstructionBase.setproperties`/`constructorof` reconstruct correctly through the public constructor.
...
- **Knot curves default to flat-forward long-end extrapolation.** Pass `extrapolation=:flat_zero`, `:linear`, or `:extension` to `Yield.Spline`, `ZeroRateCurve`, or spline `fit` methods to select another policy. `:extension` restores the former DataInterpolations final-piece continuation and is unavailable for `Spline.MonotoneConvex()`.
- **`Yield.FlatForwardAt(f)` supplies an independent terminal forward.** Bare numbers are continuously compounded; `FinanceCore.Rate` inputs are converted from their stated convention. The normalized rate stays fixed through fitting and Accessors updates. It preserves the final-knot discount factor and usually introduces a forward jump. The default `:flat_forward` instead recomputes its anchor from the chosen interpolant's endpoint derivative; changing interpolants can materially change tail levels and sensitivities (see [Interpolation Methods](interpolation.md)).
- **MonotoneConvex supports tail policies natively.** Direct construction and `fit(Spline.MonotoneConvex(), …)` return a native `Yield.MonotoneConvex` for `:flat_forward`, `:flat_zero`, `:linear`, and `Yield.FlatForwardAt(f)`, including the `Fit.Bootstrap()` descriptor path. `.rates`, `.times`, and `Yield.instantaneous_forward` remain available. The type-parameter list is internal; dispatch on `Yield.MonotoneConvex` and use its documented properties and methods. At the final knot, `instantaneous_forward` reports the interior (left) value.
- **`Yield.MonotoneConvex` equality is structural.** `==`, `isequal`, and `hash` use `(rates, times, extrapolation)`, excluding derived caches. Independently reconstructed curves with equal public inputs now compare equal; `isequal` retains the signed-zero distinction of the underlying rates.
- **`zero(::ZeroRateCurve, t)` delegates to the cached model.** It preserves the native zero-rate limit at zero and remains finite when discount factors underflow at long horizons. Negative times still raise `DomainError`.
- **`Yield.MonotoneConvex(rates, times)` accepts any iterable of reals** (ranges, tuples, `Int`s — previously `Vector` only), copies and promotes them, and applies the same validation. **`c.rates`, `c.times`, `c.f` and `c.fᵈ` are read-only** (indexed assignment throws; `copy(c.rates)` for a mutable copy) so the cached forwards can no longer be desynchronised from the knots. `propertynames(c)` lists `(:rates, :times, :extrapolation)`; the derived `f`/`fᵈ` are still readable and still listed by `propertynames(c, true)`. `@set c.rates[i] = …` / `@set c.times = …` re-validate and recompute the forwards; `@set c.f = …` / `@set c.fᵈ = …` throw instead of being silently discarded.
