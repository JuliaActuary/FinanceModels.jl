...
`copy(zrc.rates)` for a mutable copy. The interpolation cache is internal: it is not a public
property, and code that reaches it with `getfield` relies on unsupported internals.
...
once per step. The cache is built from `(rates, tenors, spline)` and is ignored by `==`,
`isequal` and `hash`.
...
struct ZeroRateCurve{R, T, S <: Sp.SplineCurve, M} <: AbstractYieldModel
    ...
    _model::M                  # built from (rates, tenors, spline); internal, not settable
    ...
    # the public API cannot put `_model` out of step with `(rates, tenors, spline)`. There is deliberately no
...
function Base.getproperty(z::ZeroRateCurve, s::Symbol)
    s === :_model && throw(
        ArgumentError(
            "ZeroRateCurve: `_model` is an internal cache derived from rates, tenors and spline; use `discount`/`zero`."
        )
    )
    return getfield(z, s)
end
...
# Structural equality on the value-carrying fields. `_model` is rebuilt from
# `(rates, tenors, spline)` by the only constructor and the public storage is read-only,
# so it is not compared. `isequal` is defined fieldwise (not via `==`) so that
