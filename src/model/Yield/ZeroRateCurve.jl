...
the last tenor while preserving the last-tenor discount factor. With `Spline.MonotoneConvex()`
the constant is its boundary instantaneous forward, so the forward curve stays continuous.
With the DataInterpolations-backed splines it is the last discrete forward,
...
final DataInterpolations polynomial piece. [`FlatForwardAt`](@ref) holds a user-specified
forward rate fixed, including when knot rates are fitted or changed.
...
(`zrc.rates[1] = …`, `.=`, `sort!`, …) throws. Use `copy(zrc.rates)` for a mutable copy.
The interpolation cache is internal: it is not a public property, and code that reaches it
with `getfield` relies on unsupported internals.
...
once per step. The cache is built from `(rates, tenors, spline, extrapolation)` and is
ignored by `==`, `isequal` and `hash`.
...
struct ZeroRateCurve{R, T, S <: Sp.SplineCurve, M, E} <: AbstractYieldModel
    ...
    _model::M                  # built from the four public fields; internal, not settable
    ...
    # the public API cannot put `_model` out of step with its public inputs. There is
    # deliberately no positional cache constructor: Accessors/ConstructionBase reconstruct through
    # `setproperties`/`constructorof` (see src/fit.jl), both of which route back here.
    function ZeroRateCurve(
            rates, tenors, spline::Sp.SplineCurve;
...
function Base.getproperty(z::ZeroRateCurve, s::Symbol)
    s === :_model && throw(
        ArgumentError(
            "ZeroRateCurve: `_model` is an internal cache derived from rates, tenors, spline and extrapolation; use `discount`/`zero`."
        )
    )
    return getfield(z, s)
end
...
# Structural equality on the value-carrying fields. `_model` is rebuilt from the public
# fields by the only constructor and the public storage is read-only, so it is not
# compared. `isequal` is defined fieldwise (not via `==`) so that
