...
construction from `(rates, times)`. All four are exposed as **read-only** vectors: indexed
assignment (`c.rates[1] = …`, `.=`, `sort!`, …) throws, so the public API cannot put the
cached forwards out of step with the knots. Use `copy(c.rates)` for a mutable copy. (This is
conventional privacy; raw `getfield` access to the backing storage is unsupported.)
